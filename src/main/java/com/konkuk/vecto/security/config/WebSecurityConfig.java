...
import com.konkuk.vecto.security.config.filter.CustomAuthenticationFilter;
import com.konkuk.vecto.security.config.filter.JwtAuthorizationFilter;
...
public class WebSecurityConfig {
    ...
    public WebSecurityCustomizer webSecurityCustomizer() {
        ...
        return web -> web.ignoring().requestMatchers(PathRequest.toStaticResources().atCommonLocations())
            .and()
            .ignoring().requestMatchers(HttpMethod.POST,"/user").requestMatchers("/**");
    }
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                // [STEP5] Spring Security Custom Filter Load - '인증'에 대해서 사용
    ...
    /**
     * 5. 커스텀을 수행한 '인증' 필터로 접근 URL, 데이터 전달방식(form) 등 인증 과정 및 인증 후 처리에 대한 설정을 구성하는 메서드입니다.
     *
    ...
    /**
     * 6. Spring Security 기반의 사용자의 정보가 맞을 경우 수행이 되며 결과값을 리턴해주는 Handler
     *
    ...
    /**
     * 7. Spring Security 기반의 사용자의 정보가 맞지 않을 경우 수행이 되며 결과값을 리턴해주는 Handler
     *
