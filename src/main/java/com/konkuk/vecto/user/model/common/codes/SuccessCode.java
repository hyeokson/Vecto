...
public enum SuccessCode {
    ...
    COMMENT_END(200, "S015", "댓글을 모두 불러왔습니다."),

    // 피드에 좋아요가 이미 등록 상태임을 나타내는 코드(HTTP Response: 200 OK)
    LIKES_ALREADY_INSERT(201, "S016", "좋아요 이미 등록 상태"),
    // 피드에 좋아요가 이미 해제 상태임을 나타내는 코드(HTTP Response: 200 OK)
    LIKES_ALREADY_DELETE(200, "S017", "좋아요 이미 해제 상태"),

    // 댓글에 좋아요 등록 성공 코드(HTTP Response: 200 OK)
    COMMENT_LIKES_INSERT(201, "S018", "댓글에 좋아요 등록 성공"),
    // 댓글에 좋아요 해제 성공 코드(HTTP Response: 200 OK)
    COMMENT_LIKES_DELETE(200, "S019", "댓글에 좋아요 해제 성공"),

    // 댓글에 좋아요가 이미 등록 상태임을 나타내는 코드(HTTP Response: 200 OK)
    COMMENT_LIKES_ALREADY_INSERT(201, "S020", "뎃글에 좋아요 이미 등록 상태"),
    // 댓글에 좋아요가 이미 해제 상태임을 나타내는 코드(HTTP Response: 200 OK)
    COMMENT_LIKES_ALREADY_DELETE(200, "S021", "댓글에 좋아요 이미 해제 상태"),

    COMMENT_DELETE(200, "S022", "댓글 삭제 성공"),
    COMMENT_PATCH(200, "S023", "댓글 수정 성공"),

    // 팔로우 등록 성공 코드(HTTP Response: 201 Inserted)
    FOLLOW_INSERT(201, "S024", "팔로우 등록 성공"),
    // 팔로우 해제 성공 코드(HTTP Response: 200 OK)
    FOLLOW_DELETE(200, "S025", "팔로우 해제 성공"),

    // 팔로우 이미 등록 성공 코드(HTTP Response: 201 Inserted)
    FOLLOW_ALREADY_INSERT(201, "S026", "팔로우 이미 등록 상태"),
    // 팔로우 이미 해제 성공 코드(HTTP Response: 200 OK)
    FOLLOW_ALREADY_DELETE(200, "S027", "팔로우 이미 해제 상태"),

    FOLLOW_RELATION(200, "S028", "유저들과의 팔로우 관계 조회 성공"),
    ...
    PUSH_NOTIFICATION_END(200, "S035", "푸쉬 알림 기록을 모두 불러왔습니다."),

    COMPLAINT_SAVE(200, "S036", "신고 접수 성공"),

    NEW_PUSH_NOTIFICATION(200, "S037", "새로운 알림 기록 여부 조회 성공"),

    FOLLOWER_USERID(200, "S038", "팔로워 userId 조회 성공"),
    FOLLOWED_USERID(200, "S039", "팔로우를 받는 userId 조회 성공")
